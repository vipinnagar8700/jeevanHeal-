...
import Addsurgeries from './components/surgeries/addsurgeries'
...
const AppUniversal = function (props) {
	...
	const toggleMobileMenu = () => {
		...
		useMemo(() => { 
		     if(location?.pathname == "/admin/login" || location?.pathname == "/admin/register" || location?.pathname == "/admin/forgotPassword" || location?.pathname == "/admin/lockscreen" || location?.pathname == "/admin/conform-email" || location?.pathname == "/admin/404" || location?.pathname == "/admin/500") {
					setIsAuth("admin")
				} else {
					setIsAuth("user")
				}
    ...
    return (
			...
			<Router basename={`${config.publicPath}`}>
				...
				<Switch>
					...
					<Route path="/admin/appointmentrepot" exact component={AppointmentReport} />
					...
					<Route path="/admin/add-surgeries" exact component={Addsurgeries} />
					<Route path="/admin/surgeries-list" exact component={Surgeries} />
					<Route path="/admin/banner" exact component={Banner} />
					<Route path="/admin/edit-banner/:b_id" exact component={Editbanner} />
					...
					<Route path="/admin/addprofile" exact component={AddProfile} />						
